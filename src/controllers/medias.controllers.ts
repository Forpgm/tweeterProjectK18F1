...
export const serveVideoStreamController = async (req: Request, res: Response) => {
  ...
  //lay kich thuoc toi da cua video
  const videoPath = path.resolve(UPLOAD_VIDEO_DIR, namefile)
  const videoSize = fs.statSync(videoPath).size //statSync lay trang thai cua video
  //neu khong co range thi yeu cau range
  if (!range) {
    return res.status(HTTP_STATUS.BAD_REQUEST).send('Requires range header')
  }
  const CHUNK_SIZE = 10 ** 6 //1mb
  const start = Number(range.replace(/\D/g, '')) //lay so trong range
  const end = Math.min(start + CHUNK_SIZE, videoSize - 1) //lay phan tu cuoi cung
  //dung luog thuc te cua video
