...
const PASSWORD = 'pgm123!'
// ID của tài khoản của mình, dùng để follow người khác
const MYID = new ObjectId('671e3cdf5598dd5d6c9f7ed0')
...
const checkAndCreateHashtags = async (hashtags: string[]) => {
  const hashtagDocuments = await Promise.all(
    hashtags.map(async (hashtag) => {
      const existingHashtag = await databaseService.hashtags.findOne({ value: hashtag })
      if (existingHashtag) {
        return existingHashtag._id
      } else {
